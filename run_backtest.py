#!/usr/bin/env python3
"""
Back-test a saved RF or DQN model on 1-minute BTC/USDT data.

2025-07-27
• Uses env.get_performance_stats() for trades & win-rate.
• Records timestamps so testing-period printout is correct.
"""
...
from utils.metrics import (
    bootstrap_equity, compute_sharpe_ratio,
    compute_signal_classification_metrics,
)
from config import (
    HIST_DAYS, PAIR, MODEL_DIR,
    THRESH_UP, THRESH_DN, PROBA_GAP,
    print_config
)
...
def main():
    ...
    ap.add_argument("--model"); args = ap.parse_args()
    print_config()
    ...
    while True:
        if is_rl:
            action, _ = agent.predict(obs, deterministic=True); action=int(action)
        else:
            p = rf.predict_proba(raw.iloc[env.pointer-1:env.pointer])[0,1]
            gap = abs(p-0.5)
            action = 1 if p>THRESH_UP and gap>=PROBA_GAP else 2 if p<THRESH_DN and gap>=PROBA_GAP else 0
    ...
    acc, mp, mr, per = compute_signal_classification_metrics(np.array(acts), pd.Series(prices,index=ts))
    ...
    for lbl,(prc,rec) in per.items():
        nm={1:'Long',-1:'Short',0:'Hold'}[lbl]
        print(f"    • {nm:5s} — precision: {prc:.2%}, recall: {rec:.2%}")
