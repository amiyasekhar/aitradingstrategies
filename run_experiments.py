...
# --- UPDATED: Added XRP/USDT to the list of assets to test ---
SYMBOLS_TO_TEST = ['BTC/USDT', 'ETH/USDT', 'SOL/USDT', 'XRP/USDT']
...
EXPERIMENT_CONFIGS = {
    ...
    '1d': {
        'max_hold_bars': 5, 'threshold': 0.005, 'stop_loss': -0.025,
        'take_profit_pct': 0.05, 'prob_threshold': 0.55, 'atr_threshold': 5000.0
    },
}
...
if __name__ == "__main__":
    ...
    for symbol in SYMBOLS_TO_TEST:
        for timeframe, params in EXPERIMENT_CONFIGS.items():
            print(f"\n{'='*50}")
            print(f"RUNNING EXPERIMENT FOR SYMBOL: {symbol}, TIMEFRAME: {timeframe}")
            print(f"PARAMETERS: {params}")
            print(f"{'='*50}\n")
            
            current_params = params.copy()
            # --- UPDATED: Added example asset-specific parameters for XRP ---
            if symbol == 'ETH/USDT':
                if timeframe == '1d': current_params['atr_threshold'] = 300.0
            if symbol == 'SOL/USDT':
                if timeframe == '1d': current_params['atr_threshold'] = 10.0
            if symbol == 'XRP/USDT':
                if timeframe == '1d': current_params['atr_threshold'] = 0.1 

            avg_results = run_wfo_for_timeframe(symbol, timeframe, current_params)
            
            if avg_results is not None:
                avg_results['symbol'] = symbol
                avg_results['timeframe'] = timeframe
                all_results.append(avg_results)

    if all_results:
        summary_df = pd.DataFrame(all_results)
        summary_df = summary_df.set_index(['symbol', 'timeframe'])
